...
class FormeGeometrice:
    ...
    def __str__(self):
        return(f"{self.forma} este {self.culoare} si este {'plin' if self.is_filled else 'nu este plin'}")


class Cerc(FormeGeometrice):

    def __str__(self):
        return (f"{self.forma} este o formă geometrică definită ca ansamblul tuturor punctelor dintr-un plan care sunt la aceeași distanță de un punct fix numit centru. ")
